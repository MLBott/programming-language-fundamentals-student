...
object fp1 {
  ...
  def max (xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    def checkMax ( xs : List[Int], maxInt : Int ) : Int = {
      xs match {
        case List() => maxInt
        case tempList :: tempListInt => if ( tempList > maxInt ) {
          checkMax( tempListInt, tempList )
          
        } else {
          checkMax( tempListInt, maxInt )
        }
      }
    }
    checkMax( xs, xs.head )
  ...
  def maxTailRecursiveAux (accumulator : Int, xs : List[Int]) : Int = {
    ...
    xs match {
      case Nil => accumulator
      case tempInt :: tempListInt => {
        if ( tempInt > accumulator ) {
          maxTailRecursiveAux( tempInt, tempListInt )
        } else {
          maxTailRecursiveAux( accumulator, tempListInt )
        }
      }
    }
  }
  ...
  def otpu (start : Int, end : Int) : List[Int] = {
    ...
    if (start < end) {
      ...
      val mainList: List[Int] = start :: otpu (start - 1, end)
      mainList
    } else if (start == end) {
  ...
  def main(args: Array[String]): Unit = {
    println("Hello, Galactic Core!!!")
    println(factTest)
    println(fib(10))
    println(p1)
    println(t1)
    println(swap(7, "hello"))
    println(sum(List(2,3,4)))
    println(sumTailRecursive(List(2,3,4)))
    println(max(List(2,3,4,7,5,6)))
    println(maxTailRecursive(List(2,4,8,11)))
    println(otpu(55,12))
