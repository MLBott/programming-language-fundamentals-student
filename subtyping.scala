...
object subtyping {
  ...
  def observeCounter (f : Counter => Unit) : Int = {
    ...
    class internalCount extends Counter {
      private var i, j = 0    
  ...
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    ...
    val k = new internalCount; val l = new internalCount; val m = new internalCount
    f(List(k, l, m))
    val a = List(k.get, l.get, m.get)
    a
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    ...
    val k = new internalCount; val l = new internalCount; val m = new internalCount
    val countArray : Array[Counter] = Array(k, l, m)
    f(countArray)
    val a = List(k.get, l.get, m.get).toArray
    a
  }
}
