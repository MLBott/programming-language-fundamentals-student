...
object fp2 {
  ...
  def joinTerminateLeft (xs : List[String], term : String) : String = {
    // TODO: Provide definition here.
    def i(a: String, b: String) : String = a + b + term
    xs match {
      case Nil => ""
      case y :: ys => y + foldLeft(ys,term,i)
    }
  }
  ...
  def joinTerminateRight (xs : List[String], delimiter : String) : String = {
    // TODO: Provide definition here.
    def i(a: String, b: String) : String = delimiter + a + b
    xs match {
      case Nil => ""
      case y :: ys => y + foldRight(ys,delimiter,i)
    }
  }
  ...
  def firstIndexNumGreaterThan (a : Int, xs : List[Int]) : Int = {
    // TODO: Provide definition here.
    xs.indexOf(firstIndexNumGAux(a,xs))
  }
  
  def firstIndexNumGAux(a:Int,xs:List[Int]) : Int = {
    xs match {
      case Nil => throw new RuntimeException()
      case x :: xs => if(x < a) firstIndexNumGAux(a,xs) else x
    }
  }
